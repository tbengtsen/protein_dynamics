...
if __name__ =="__main__":
    ...
    # max equil time before discarding the protein
    equil_2_max = 7.0 * unit.nanoseconds


    # first assessing (energy+rmsd) equilibration after:
    assess_equil_2 = 5 * unit.nanoseconds
    ...
    equil_max_rmsd = 5.0 * unit.angstrom  # a bit high but important for large flex tails or loops
    ...
    while equilibrated is False:
        ...
        if curr_time >= assess_equil_1:
            ...
            if equilibrated:
                ...
                out_file = f"equil_1_{name_pdb}.pdb"
    ...
    out_file = f"equil_2_{name_pdb}.pdb"
    save_pdb(curr_state, modeller, out_file, out_dir)
    ...
    integrator = integrate_method(temperature, friction, sim_dt)
    simulation = Simulation(modeller.topology, system, integrator, platform,
            platform_properties)
    simulation.context.setPositions(modeller.positions)
