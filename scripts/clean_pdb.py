...
def _pdbfixer(first_model, temp3):
    ...
    #fixer.addMissingHydrogens(7.0) # adds hydrogens incorrectly on aromatics 
    
    return temp3, fixer


def _fix_numbering(fixer, temp4, temp5):
    '''
    Correct for pdbfixer not preserving insertion codes
    '''
    simtk.openmm.app.PDBFile.writeFile(
        fixer.topology, fixer.positions, temp5, keepIds=False
    )
    temp5.flush()
